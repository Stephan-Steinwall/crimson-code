...
const FAQ = () => {
  ...
  return (
    <div className="scroll-mt-[125px] my-10 max-w-7xl mx-auto p-8" id='faq'>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          {faqs.slice(0, 3).map((faq, index) => (
            <div
              key={index}
              className={`rounded-3xl overflow-hidden transition-all duration-300 cursor-pointer ${faq.bgColor}`}
              onClick={() => toggleFaq(index)}
            >
              <div className="p-6">
                <div className="flex justify-between items-center gap-4">
                  <h3 className={`font-bold text-xl ${faq.textColor || 'text-black'}`}>{faq.question}</h3>
                  <div className={`${faq.bgColor === 'bg-orange-500' ? 'bg-white' : 'bg-orange-50'} p-2 rounded-lg`}>
                    <svg 
                ...
                >
                  <p className={`${faq.textColor || 'text-gray-600'}`}>{faq.answer}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="space-y-6">
          {faqs.slice(3).map((faq, index) => (
            <div
              key={index + 3}
              className={`rounded-3xl overflow-hidden transition-all duration-300 cursor-pointer ${faq.bgColor} hover:shadow-lg`}
            ...
            >
              <div className="p-6">
                ...
                <div 
                  className={`overflow-hidden transition-all duration-300 ${
                    activeIndex === index + 3 ? 'max-h-40 mt-4 opacity-100' : 'max-h-0 opacity-0'
                  }`}
                >
